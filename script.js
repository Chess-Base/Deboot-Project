document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedPersonality = localStorage.getItem("personality") || "teacher";
...
let currentPersonality = "teacher";
...
function loadChatHistory() {
  ...
  if (savedHistory) {
    ...
    appendMessage("bot", getPersonalityGreeting());
  }
...
function appendMessage(sender, message, model = null, note = null) {
  ...
  if (sender === 'bot' && typeof message === 'string') {
    ...
    contentDiv.innerHTML = addPersonalityEmojis(contentDiv.innerHTML);
  } else {
...
function getPersonalityGreeting() {
  switch (currentPersonality) {
    case "teacher": return "Hello, student! Ready to learn something new today? 📚✨";
    case "lover": return "Hey there, gorgeous 😘 Ready to have some fun with me? 💋";
    case "friend": return "Yo, what’s up? Ready to chill and chat? 😎👍";
    default: return "Greetings! I’m DEBoot, your professional AI assistant.";
  }
}

function addPersonalityEmojis(text) {
  switch (currentPersonality) {
    case "teacher": return text + " ✅📝";
    case "lover": return text + " 😘💖";
    case "friend": return text + " 😊👊";
    default: return text;
  }
}
...
function updatePersonality() {
  ...
  if (personalitySelect) {
    ...
    showNotification(`Personality updated to ${currentPersonality}!`, "success");
  }
}
...
function determineBestModel(query) {
  ...
  switch (queryType) {
    case "code": return "deepseek-chat";
    case "math": return "claude-3-7-sonnet";
...
async function sendMessage(message = null) {
  ...
  let instruction = getPersonalityInstruction(queryType);
...
function getPersonalityInstruction(queryType) {
  switch (currentPersonality) {
    case "teacher":
      return queryType === "math" 
        ? "Solve this quickly and accurately with a clear explanation. 📚" 
        : "Provide a concise, correct answer like a teacher would. ✏️";
    case "lover":
      return "Answer seductively and playfully, darling. 😘 Keep it flirty and fun!";
    case "friend":
      return "Keep it chill and friendly, bro. 😎 Give a laid-back, cool response.";
    default:
      return "Respond in a professional tone with clear, concise answers.";
  }
}
